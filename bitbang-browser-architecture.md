...
self.addEventListener('fetch', (event) => {
    ...
    // Don't intercept the bootstrap page itself or the SW registration
    if (url.pathname === '/bootstrap' || url.pathname === '/sw.js') {
        return;
    }
...
async function fetchViaDataChannel(event) {
    ...
    const bootstrap = allClients.find(c =>
        c.url.includes('/bootstrap') || c.frameType === 'top-level'
    );
